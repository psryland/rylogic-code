...
namespace pr::fluid
{
	struct Scene2d : IDemoScene
	{
			...
			, m_dynamics(particle_count)
		{
			...
			// Floor
			m_ldr.Plane("floor", 0xFFade3ff).wh({ 2, 0.5f }).o2w(m4x4::Transform(Rotation<m3x3>(AxisId::PosZ, AxisId::PosY), v4{ 0, -1, 0, 1 }));
			m_col.Plane().o2w(m3x3::Rotation(AxisId::PosZ, AxisId::PosY), v4{ 0, -1, 0, 1 });

			// Ceiling
			m_ldr.Plane("ceiling", 0xFFade3ff).wh({ 2, 0.5f }).o2w(m4x4::Transform(Rotation<m3x3>(AxisId::PosZ, AxisId::NegY), v4{ 0, +1, 0, 1 }));
			m_col.Plane().o2w(m3x3::Rotation(AxisId::PosZ, AxisId::NegY), v4{ 0, +1, 0, 1 });

			// Left Wall
			m_ldr.Plane("left_wall", 0xFFade3ff).wh({ 0.5f, 2 }).o2w(m4x4::Transform(Rotation<m3x3>(AxisId::PosZ, AxisId::PosX), v4{ -1, 0, 0, 1 }));
			m_col.Plane().o2w(m3x3::Rotation(AxisId::PosZ, AxisId::PosX), v4{ -1, 0, 0, 1 });

			// Right Wall
			m_ldr.Plane("right_wall", 0xFFade3ff).wh({ 0.5f, 2 }).o2w(m4x4::Transform(Rotation<m3x3>(AxisId::PosZ, AxisId::NegX), v4{ +1, 0, 0, 1 }));
			m_col.Plane().o2w(m3x3::Rotation(AxisId::PosZ, AxisId::NegX), v4{ +1, 0, 0, 1 });
