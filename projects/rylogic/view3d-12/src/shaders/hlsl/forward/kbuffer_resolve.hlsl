...
PSIn VSFullScreenTriangle(uint vid :SV_VertexID)
{
	PSIn Out = (PSIn)0;
...
float PSDepthResolve(PSIn In) :SV_Depth
{
...
float4 PSAlphaResolve(PSIn In) :SV_Target
{
	int2 pix = int2(In.ss_vert.xy);
	...
	base.rgb = base.rgb * (1.0f - oia.a) + oia.rgb;
