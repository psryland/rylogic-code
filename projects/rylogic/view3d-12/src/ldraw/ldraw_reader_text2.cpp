...
namespace pr::rdr12::ldraw
{
	namespace
	{
		...
		// Convert Reader2 include flags to the shared path-resolver flags.
		...
		// Owns converted include bytes while exposing Reader2's bulk input contract.
		struct OwnedUtf8Input : script::v2::IInput
		{
			std::string m_source;
			size_t m_position;
			...
			// Copy the next UTF-8 block into Reader2's transport buffer.
		...
		// Bridge LDraw's resolver to Reader2's UTF-8 include contract.
		struct IncludeAdapter : script::v2::IIncludeHandler2
		{
			...
			// Snapshot and convert each include before Reader2 begins scanning it.
			std::unique_ptr<script::v2::IInput> Open(std::filesystem::path const& resolved, script::EIncludeFlags flags, script::Loc const&) override
			{
		...
		// True when a compact object-header token is a valid legacy LDraw name.
	...
	std::string ToUtf8Source(std::string_view source, EEncoding encoding, std::filesystem::path const& filepath)
	{
		...
		if (encoding == EEncoding::auto_detect)
			...
			bom_size = static_cast<int>(script::v2::Utf8BomLength(source));
	...
	struct TextReader2::Impl
	{
		...
		std::unique_ptr<script::v2::Reader> m_reader;
		...
		// Access the preprocessed byte stream underlying the Reader2 facade.
		script::v2::Preprocessor& Source()
		{
	...
	TextReader2::TextReader2(std::istream& utf8_stream, std::filesystem::path src_filepath, ReportErrorCB report_error_cb, ParseProgressCB progress_cb, IPathResolver const& resolver)
		: IReader(report_error_cb, progress_cb, resolver)
		, m_impl(std::make_unique<Impl>(utf8_stream, std::move(src_filepath), resolver))
	{
	}

	TextReader2::~TextReader2() = default;
	...
	bool TextReader2::NextKeywordImpl(int& kw)
	{
	...
	string32 TextReader2::IdentifierImpl(bool incl_dot)
	{
	...
	int64_t TextReader2::IntImpl(int, int radix)
	{
	...
	int64_t TextReader2::EnumImpl(int, ParseEnumIdentCB parse)
	{
	...
	string32 TextReader2::LastKeywordString() const
	{
