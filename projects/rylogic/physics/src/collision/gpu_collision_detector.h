...
namespace pr::physics
{
	struct GpuCollisionDetector
	{
		...
		D3DPtr<ID3D12Resource> m_r_dispatch;      // GPU buffer: RWStructuredBuffer<D3D12_DISPATCH_ARGUMENTS> for indirect dispatch of the resolve shader based on the number of contacts generated by CD
		...
		void DetectCollisions(GpuJob& job, int max_contacts, D3DPtr<ID3D12Resource> dispatch, D3DPtr<ID3D12Resource> pairs, D3DPtr<ID3D12Resource> counters, CollisionShapeCache const& shape_cache, std::span<GpuMaterial const> materials);
		int DetectCollisions(GpuJob& job, std::span<GpuCollisionPair const> pairs, CollisionShapeCache const& shape_cache, std::span<GpuMaterial const> materials, std::span<GpuResolveContact> contacts);

		// Get the GPU resources
		D3DPtr<ID3D12Resource> Contacts() { return m_r_contacts; }
		D3DPtr<ID3D12Resource> DispatchArgs() { return m_r_dispatch; }
	...
	private:
		...
		bool ResizeBuffers(CmdList& cmd_list, int max_contacts, int max_shapes, int max_verts, int max_materials);
	};
}
