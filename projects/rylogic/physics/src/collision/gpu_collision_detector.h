...
namespace pr::physics
{
	struct GpuCollisionDetector
	{
		...
		void DetectCollisions(GpuJob& job, int max_contacts, D3DPtr<ID3D12Resource> dispatch, D3DPtr<ID3D12Resource> pairs, D3DPtr<ID3D12Resource> counters, ShapeCache const& shape_cache, std::span<GpuMaterial const> materials);
		int DetectCollisions(GpuJob& job, std::span<GpuCollisionPair const> pairs, ShapeCache const& shape_cache, std::span<GpuMaterial const> materials, std::span<GpuResolveContact> contacts);
