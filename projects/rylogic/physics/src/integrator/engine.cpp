...
namespace pr::physics
{
	...
	Engine::Engine(EngineConfig const& config, IShaderCache* shader_cache, ID3D12Device4* existing_device)
		...
		, m_gpu_integrator(new GpuIntegrator(*m_gpu, m_config, shader_cache))
		, m_gpu_sleep_manager(new GpuSleepManager(*m_gpu, m_config, shader_cache))
		, m_gpu_sort_and_sweep(new GpuSortAndSweep(*m_gpu, m_config, shader_cache))
		, m_gpu_collision_detector(new GpuCollisionDetector(*m_gpu, m_config, shader_cache))
		...
		, m_gpu_selective_refresher(new GpuSelectiveRefresher(*m_gpu, m_config, shader_cache))
