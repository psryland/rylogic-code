...
namespace pr::physics::tests
{
	namespace
	{
		...
		// Compile final-configuration coupled rows and exact-self factors using the same replay kernels as production.
		CoupledVelocityInputs MakeCoupledVelocityInputs(CoupledVelocityFixture& fixture)
		{
			...
			auto constraint_upload = PackGpuConstraints(fixture.m_constraints, remap);
			...
			link_to_world.reserve(fixture.m_articulation.LinkCount());
			for (int link_idx = 0; link_idx != fixture.m_articulation.LinkCount(); ++link_idx)
				link_to_world.push_back(PackGpuTransform(fixture.m_articulation.LinkToWorld(fixture.m_articulation.LinkAt(link_idx))));

			auto packed_bodies = std::vector<GpuRigidBody>{PackDynamics(fixture.m_body, 0)};
	...
	PRUnitTestClass(CoupledConstraintVelocityGpuTests)
	{
		...
		PRUnitTestMethod(HardwareMatchesReplayAndOptionalCost, Extended)
		{
			...
			PR_EXPECT(solver.Stats().m_dispatch_count == 9);
