...
int main(int argc, char* argv[])
{
	...
	auto families = std::vector<std::string_view>{};
	for (auto i = 1; i != argc; ++i)
	{
		if (strcmp(argv[i], "-verbose") == 0)
			...
			families.push_back(arg.substr(std::string_view{"-family:"}.size()));
		else
			filters.push_back(argv[i]);
	}
	return pr::unittests::RunAllTests(wordy, filters, excludes, families);
}
