param(
	...
	[string]$OutputCsv = "projects\tests\physics-sandbox\dump\tgs_pyramid_benchmark.csv"
)
...
{
	param([int]$TgsStepCount)

	$log = Join-Path $env:APPDATA "RylogicPhysicsSandbox\scene_diagnostic.log"
	$out = Join-Path $env:TEMP "rylogic_tgs_${TgsStepCount}_$([guid]::NewGuid()).out"
	$err = Join-Path $env:TEMP "rylogic_tgs_${TgsStepCount}_$([guid]::NewGuid()).err"
	...
	if ($null -ne $process.ExitCode -and $process.ExitCode -ne 0)
	{
		...
		throw "TGS benchmark run failed for tgs_steps=$TgsStepCount exit=$($process.ExitCode)`n$stderr"
	}
	...
	{
		$stderr = if (Test-Path $err) { Get-Content $err -Raw } else { "" }
		throw "Scene diagnostic log was not created for tgs_steps=$TgsStepCount`n$stderr"
	}
	...
	{
		throw "No pyramid metric rows found for tgs_steps=$TgsStepCount"
	}
	...
	foreach ($line in $pyramid_lines)
	{
		...
		if (!$match.Success)
		{
			throw "Could not parse pyramid metric row for tgs_steps=$TgsStepCount`n$line"
		}
...
$tgs_step_counts = Convert-ToIntList $TgsSteps
$results = @()
foreach ($tgs_step_count in $tgs_step_counts)
{
	$results += Invoke-PyramidMetricRun $tgs_step_count
}

$baseline = $results | Where-Object { $_.TgsSteps -eq 1 } | Select-Object -First 1
if ($null -eq $baseline)
{
	throw "TgsSteps must include 1 so the benchmark has a non-TGS baseline"
}

$summary = foreach ($result in $results)
{
	# Compare every run to tgs_steps=1: lower whole-pyramid RMS pose error is better, lower physics time is cheaper.
...
$summary | Format-Table -Property TgsSteps,FinalStep,CostStep,RmsPose,StabilityGainPct,MaxXY,Fallen,Sleeping,PhysicsMs,Fps,CostRatio -AutoSize
Write-Host ""
Write-Host "TGS benchmark CSV: $([System.IO.Path]::GetFullPath($OutputCsv))"
