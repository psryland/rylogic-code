...
namespace pr::math
{
	...
	struct Hermite3Vector
	{
		...
		Hermite3Vector() noexcept
			:Hermite3Vector(Origin<Vec4>(), Zero<Vec4>(), Origin<Vec4>(), Zero<Vec4>(), S(1))
		{
		}
		Hermite3Vector(Vec4 x0, Vec4 v0, Vec4 x1, Vec4 v1, S interval) noexcept
	...
	struct Hermite3Quaternion
	{
		...
		Hermite3Quaternion() noexcept
			:Hermite3Quaternion(Identity<Quat>(), Zero<Vec4>(), Identity<Quat>(), Zero<Vec4>(), S(1))
		{
		}
		Hermite3Quaternion(Quat q0, Vec4 w0, Quat q1, Vec4 w1, S interval) noexcept
			: m_p()
			, m_q1(q1)
			, m_interval(interval)
		{
		...
		Vec4 EvalDerivative(S t) const noexcept
		{
			...
			return Rotate(m_q1, AngularVelocity(u, u_dot));
		}

		// Map a log-space orientation and derivative to angular velocity in the same frame.
		static Vec4 AngularVelocity(Vec4 u, Vec4 u_dot) noexcept
		{
			...
			auto f = r > S(0.1)
				? (sin_r / r)
				: (S(1) - Sqr(r) / S(6) + Sqr(Sqr(r)) / S(120));
			auto f_dot = r > S(0.1)
				? (r * cos_r - sin_r) / Sqr(r)
				: (-r / S(3) + r * Sqr(r) / S(30) - r * Sqr(Sqr(r)) / S(840));
		...
		static Vec4 Tangent(Quat q, Vec4 w) noexcept
		{
			...
			// The half-angle cotangent form remains well-conditioned at the shortest-arc boundary.
			auto alpha = r < SmallAngle
				? S(1) / S(12) + Sqr(r) / S(720) + Sqr(Sqr(r)) / S(30240)
				: (S(1) - S(0.5) * r / std::tan(S(0.5) * r)) / Sqr(r);
	...
	template <ScalarTypeFP S>
	struct Hermite3Transform
	{
		using Xform = Xform<S>;
		using Vec4 = Vec4<S>;
		using Quat = Quat<S>;

		Hermite3Vector<S> pos;
		Hermite3Quaternion<S> rot;

		Hermite3Transform() noexcept
			: pos()
			, rot()
		{
		}
		Hermite3Transform(
			Vec4 pos0, Vec4 vel0, Quat rot0, Vec4 avl0,
			Vec4 pos1, Vec4 vel1, Quat rot1, Vec4 avl1,
			S interval) noexcept
			: pos(pos0, vel0, pos1, vel1, interval)
			, rot(rot0, avl0, rot1, avl1, interval)
		{
		}
	...
	// Transitional aliases preserve the original names while callers migrate to degree-specific types.
	template <ScalarTypeFP S> using HermiteVector = Hermite3Vector<S>;
	template <ScalarTypeFP S> using HermiteQuaternion = Hermite3Quaternion<S>;
	template <ScalarTypeFP S> using HermiteTransform = Hermite3Transform<S>;

	template <ScalarTypeFP S>
	struct HermiteVector_MidPoint
	{
		// Notes:
		//  - Hermite spline through three positions: (pos_prev at -T, pos_mid at 0, pos_next at +T).
		//  - The midpoint tangent is derived from the positions at +/- T as a finite-difference: (pos_next - pos_prev) / interval.
		//    Although this may not match the true velocity at the mid point, it ensures that the interpolated path is well behaved (i.e, no loops, etc).
		using CurveType = CurveType<S>;
		using CubicCurve3 = CubicCurve3<S>;
		using Vec4 = Vec4<S>;
			...
			: pos()
		{
			...
			pos.m_p = CubicCurve3(
				pos_prev - pos_next,
				S(4) * pos_mid - S(3) * pos_prev - pos_next,
				Zero<Vec4>(),
				pos_prev + S(3) * pos_next - S(4) * pos_mid,
				CurveType::Hermite
			);
			pos.m_x1 = pos_next;
		...
		// Evaluate position at time t (t=0 at midpoint, t=-T at pos_prev, t=+T at pos_next, where T = interval/2)
		Vec4 Eval(S t) const noexcept
		{
		...
		// Evaluate velocity at time t (in world-space units per second)
		Vec4 EvalDerivative(S t) const noexcept
		{
			S T = S(0.5) * pos.m_interval;
			S u = (t + T) / pos.m_interval;
		...
		// Evaluate acceleration at time t (in world-space units per second^2)
		Vec4 EvalDerivative2(S t) const noexcept
		{
			S T = S(0.5) * pos.m_interval;
			S u = (t + T) / pos.m_interval;
			return pos.m_p.EvalDerivative2(u) / Sqr(pos.m_interval);
		}
	};
	...
	{
		// Notes:
		//  - Hermite interpolation for quaternions through three orientations: (rot_prev at -T, rot_mid at 0, rot_next at +T).
		//  - Uses Hermite interpolation in SO(3) log space with angular velocity derived from the orientations.
		//  - The midpoint angular velocity is derived from rot_prev and rot_next using log-map finite differences,
		//    avoiding the velocity/orientation mismatch problem of using stored angular velocities.
		using Hermite3Quaternion = math::Hermite3Quaternion<S>;
		using CurveType = CurveType<S>;
		using CubicCurve3 = CubicCurve3<S>;
		using Quat = Quat<S>;
		using Vec4 = Vec4<S>;

		Hermite3Quaternion rot;

		HermiteQuaternion_MidPoint() noexcept
			: rot()
		{
		}
		HermiteQuaternion_MidPoint(Quat rot_prev, Quat rot_mid, Quat rot_next, S interval) noexcept
			: rot()
		{
			// Work in the log domain relative to rot_next:
			//   u(t) = log(~rot_next * q(t)), so u0 = log(~rot_next * rot_prev), u1 = 0
			//   u_mid = log(~rot_next * rot_mid)
			//
			// Derive the midpoint angular velocity from orientations only:
			//   avl_mid = 2 * log(~rot_prev * rot_next) / interval
			auto dq0 = ~rot_next * rot_prev;
			if (vec(dq0).w < 0) dq0 = -dq0;
			auto dq_mid = ~rot_next * rot_mid;
			if (vec(dq_mid).w < 0) dq_mid = -dq_mid;

			auto u0 = LogMap<Vec4>(dq0);
			auto u_mid = LogMap<Vec4>(dq_mid);

			// Finite-difference angular velocity at midpoint
			auto dq_prev_to_next = ~rot_prev * rot_next;
			if (vec(dq_prev_to_next).w < 0) dq_prev_to_next = -dq_prev_to_next;
			auto avl_mid_world = S(2) * LogMap<Vec4>(dq_prev_to_next) / interval;
			auto t_mid = Hermite3Quaternion::Tangent(dq_mid, Rotate(~rot_next, avl_mid_world)) * interval;
	...
	{
		// Notes:
		//  - Hermite interpolation through three transforms: (prev at -T, mid at 0, next at +T).
		//  - Position uses HermiteVector_MidPoint (position-derived tangents).
		//  - Rotation uses HermiteQuaternion_MidPoint (orientation-derived angular velocity).
		using Xform = Xform<S>;
		using Vec4 = Vec4<S>;
		using Quat = Quat<S>;
		...
		HermiteTransform_MidPoint(
			Vec4 pos_prev, Vec4 pos_mid, Vec4 pos_next,
			Quat rot_prev, Quat rot_mid, Quat rot_next,
			S interval) noexcept
			: pos(pos_prev, pos_mid, pos_next, interval)
			, rot(rot_prev, rot_mid, rot_next, interval)
		{
		}
		...
		Xform Eval(S t) const noexcept
		{
			return Xform{ pos.Eval(t), rot.Eval(t), Vec4::One() };
		}

		// For derivatives, access 'pos' or 'rot' as needed.
	};
}
...
namespace pr::math::tests
{
	PRUnitTestClass(InterpolatorTests)
	{
		...
		std::span<Sample const> GenerateTestData(int n = 100000) noexcept
		{
			...
			for (int i = 1; i != ssize(samples) - 1; ++i)
			{
				...
				samples[i].avl = t_pn > 0 ? 2.0f * LogMap<V4>(ori_p1n1) / t_pn : V4{}; // LogMap returns half the angular displacement represented by the quaternion.
			}

			n = std::min<int>(n, static_cast<int>(ssize(samples)));
		...
		PRUnitTestMethod(HermiteVec, Quick)
		{
			...
			// Physical acceleration applies the second-order time scaling for non-unit intervals.
			{
				auto x0 = V4(0, 0, 0, 1);
				auto x1 = V4(8, 0, 0, 1);
				auto v0 = V4::Zero();
				auto v1 = V4::Zero();
				auto interp = HermiteVector<float>(x0, v0, x1, v1, 2.0f);
				PR_EXPECT(FEqlAbsolute(interp.EvalDerivative2(0), V4(12, 0, 0, 0), tol));
				PR_EXPECT(FEqlAbsolute(interp.EvalDerivative2(2), V4(-12, 0, 0, 0), tol));
			}
		...
		PRUnitTestMethod(HermiteXform, Quick)
		{
			...
			// Simple case: translate + rotate from identity
			{
				...
				auto rot0 = Q(0, 0, 0, 1);
				auto avl0 = V4(0, 0, constants<float>::tau_by_4, 0);
				auto pos1 = V4(1, 0, 0, 1);
				auto vel1 = V4(1, 0, 0, 0);
				auto rot1 = Q(V4::ZAxis(), constants<float>::tau_by_4);
				auto avl1 = V4(0, 0, constants<float>::tau_by_4, 0);

				HermiteTransform<float> interp(pos0, vel0, rot0, avl0, pos1, vel1, rot1, avl1, 1.0f);
			...
			for (int i = 0; i != 50; ++i)
			{
				...
				auto avl0 = Random<V4>(rng, Origin<V4>(), 3.0f).w0();
				auto avl1 = Random<V4>(rng, Origin<V4>(), 3.0f).w0();
		...
		PRUnitTestMethod(HermiteVec_MidPoint, Quick)
		{
			...
			{
				auto pos_prev = vec4_t(0, 0, 0, 1);
				auto pos_mid = vec4_t(1, 0, 0, 1);
				auto pos_next = vec4_t(2, 0, 0, 1);
				S interval = 2;

				HermiteVector_MidPoint<S> interp(pos_prev, pos_mid, pos_next, interval);
				...
				// Midpoint: Eval(0) = pos_mid
				PR_EXPECT(FEql(interp.Eval(0), pos_mid));

				// Midpoint velocity: EvalDerivative(0) = vel_mid
				auto vel_mid = (pos_next - pos_prev) / interval;
				PR_EXPECT(FEql(interp.EvalDerivative(0), vel_mid));

				// Acceleration uses physical time rather than the normalized curve parameter.
				PR_EXPECT(FEql(interp.EvalDerivative2(0), vec4_t::Zero()));
			}

			// Non-trivial curve with vertical motion
			{
				auto pos_prev = vec4_t(0, 0, 0, 1);
				auto pos_mid = vec4_t(2, 1, 0, 1);
				auto pos_next = vec4_t(4, 0, 0, 1);
				auto interval = S(2);
				...
				PR_EXPECT(FEql(interp.Eval(0), pos_mid));
				PR_EXPECT(FEql(interp.EvalDerivative(0), vel_mid));
				PR_EXPECT(FEql(interp.EvalDerivative2(0), vec4_t(0, -2, 0, 0)));
			}

			// Smoothness: verify no discontinuities by checking values at close sample points
			{
				auto pos_prev = vec4_t(0, 1, 0, 1);
				auto pos_mid = vec4_t(S(1.5), 2, S(0.5), 1);
				auto pos_next = vec4_t(3, 2, -1, 1);
				auto interval = S(2);
				auto T = interval / S(2);
				auto eps = S(0.001);
		...
		PRUnitTestMethod(HermiteXform_MidPoint, Quick)
		{
			...
			// Combined translation + rotation midpoint interpolator
			{
				...
				HermiteTransform_MidPoint<float> interp(
					pos_prev, pos_mid, pos_next,
					rot_prev, rot_mid, rot_next,
					interval);
		...
		inline static int N = 3;
		inline static float step = 0.001f;
		inline static float vel_scale = 0.1f;
		inline static float avl_scale = 0.1f;
		inline static int boxes_per_segment = 20;
		inline static V4 const box_dim = 0.5f * V4{ 1.0f, 1.5f, 2.0f, 0.0f };

		void DrawInput(ldraw::LdrGroup& grp, std::span<Sample const> samples)
		{
			...
			auto& grp_avl = grp.Group("avl_in");
			for (int i = 1; i != ssize(samples) - 1; ++i)
			{
				auto& s = samples[i];
				track.line_to(s.pos);

				grp_vel.Line("vel", 0x80FFFF00).strip(s.pos).line_to(s.pos + vel_scale * s.vel);
				grp_avl.Line("avl", 0x8000FFFF).strip(s.pos).line_to(s.pos + avl_scale * s.avl);
