	...
	PRUnitTestClass(Matrix)
	{
		...
		PRUnitTestMethod(MoveAssignmentPreservesTranspose, float, double)
		{
			...
			auto check_assignment = [&](int vecs, int cmps, std::initializer_list<T> data) noexcept
			{
				...
				// Move a normal matrix into a destination that already starts out transposed.
				auto transposed_dest = mat_t(vecs, cmps);
				transposed_dest.transpose();
				auto moved_normal = normal;
				transposed_dest = std::move(moved_normal);
				...
				// Move a transposed matrix into a destination that starts out normal.
				auto normal_dest = mat_t(cmps, vecs);
				auto moved_transposed = transposed_source;
				normal_dest = std::move(moved_transposed);

				PR_EXPECT(normal_dest.vecs() == transposed_source.vecs());
				PR_EXPECT(normal_dest.cmps() == transposed_source.cmps());
				PR_EXPECT(FEql(normal_dest, transposed_source));

				// Self-move must remain a no-op regardless of transpose state.
				auto self = transposed_source;
				self = std::move(self);
				PR_EXPECT(FEql(self, transposed_source));
			};
