...
namespace pr
{
	// Pack a normal vector into 2 components using octahedral encoding
	template <ScalarTypeFP S>
	struct OctEncoding
	{
		// Notes:
		//  - see "A Survey of Efficient Representations for Independent Unit Vectors" by Cigolle et al, 2014
		using Vec2 = math::Vec2<S>;
		using Vec3 = math::Vec3<S>;

		static Vec2 Encode(Vec3 normal) noexcept
		{
			auto const l1_norm = math::Abs(normal.x) + math::Abs(normal.y) + math::Abs(normal.z);
			if (l1_norm <= S(1e-20))
				return Vec2{ 0, 0 };

			auto n = normal / l1_norm;
			return n.z >= 0 ? n.xy : Wrap(n.xy);
		}
		static Vec3 Decode(Vec2 encoded) noexcept
		{
			auto n = Vec3{
				encoded.x,
				encoded.y,
				S(1) - math::Abs(encoded.x) - math::Abs(encoded.y)
			};

			if (n.z < 0)
				n.xy = Wrap(n.xy);

			auto const len = math::Length(n);
			return len > S(1e-20)
				? n / len
				: Vec3{ 0, 0, 1 };
		}
		static Vec2 Wrap(Vec2 value) noexcept
		{
			return Vec2{
				(1 - math::Abs(value.y))* (value.x >= 0 ? +S(1) : -S(1)),
				(1 - math::Abs(value.x))* (value.y >= 0 ? +S(1) : -S(1))
			};
		}
	};
