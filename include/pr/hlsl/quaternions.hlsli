...
static const float TinyAngle = 1e-8f;
static const float SmallAngle = 1e-5f;
...
inline float3 quat_unrotate(float4 q, float3 v)
{
...
inline float3 quat_rotation_vector(float4 q_from, float4 q_to)
{
	...
	float scale = len > 1e-6f ? (2.0f * half_angle / len) : 2.0f;
	return scale * qd.xyz;
}
...
inline float4 quat_from_float3x3(float3x3 m)
{
	float4 q;
	float trace = m[0][0] + m[1][1] + m[2][2];
	if (trace >= 0)
	{
		float s = 0.5f * rsqrt(1.0f + trace);
		q.x = (m[1][2] - m[2][1]) * s;
		q.y = (m[2][0] - m[0][2]) * s;
		q.z = (m[0][1] - m[1][0]) * s;
		q.w = 0.25f / s;
	...
	{
		float s = 0.5f * rsqrt(1.0f + m[0][0] - m[1][1] - m[2][2]);
		q.x = 0.25f / s;
		q.y = (m[0][1] + m[1][0]) * s;
		q.z = (m[2][0] + m[0][2]) * s;
		q.w = (m[1][2] - m[2][1]) * s;
...
inline float4 quat_exp(float3 v)
{
	...
	float s = ang_by_2 > TinyAngle ? (sin_half_ang / ang_by_2) : 1.0f;
...
inline float3 quat_tangent(float4 q, float3 w)
{
	// 'u' = axis * full_angle (radians)
	// 'r' = |u| = angle
	float3 u = 2.0f * quat_log(q);
	float r = length(u);
	...
	float alpha = abs(sin_r) > SmallAngle
		? (1.0f / (r * r)) - (1.0f + cos_r) / (2.0f * r * sin_r)
		: (1.0f / 12.0f);

	// J^{-1} * w = w - 0.5 * u x w + alpha * (u x (u x w))
	float3 tangent = w - 0.5f * u_x_w + alpha * u_x_u_x_w;
	return 0.5f * tangent;
}
