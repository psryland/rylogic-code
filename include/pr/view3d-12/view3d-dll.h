...
namespace pr
{
	...
	namespace view3d
	{
		...
		struct ObjectCreateOptions
		{
			...
			int m_struct_size;
			int m_version;
			EVertexSource m_vertex_source; // Buffer copies declared fields; ProceduralVertexId ignores placeholder fields and retains generated nugget capabilities.
...
extern "C"
{
	...
	// The input is one complete Vertex record. The renderer stores one canonical internal vertex with an empty input layout, not a compact position buffer.
